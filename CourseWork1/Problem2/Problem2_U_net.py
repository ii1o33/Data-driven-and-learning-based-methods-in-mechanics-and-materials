...
############################################################################################
user_hidden = 4                # number of hidden layers
user_width = 368                # neurons per hidden layer
user_lr = 0.0002536
user_BatchSize = 64
...
for train_index, val_index in kf.split(X_all):
    ...
    net = UNetFCNN(input_dim=20, hidden_layers=user_hidden, width=user_width)
    loss_func = Lossfunc()
    ...
    for epoch in range(n_epoch):
        ...
        if epoch % 10 == 0:
            print(f"Epoch {epoch}: Train Loss: {trainloss:.6f}, "
                  f"Val Loss: {valloss:.6f}, Train Acc: {trainacc:.3f}, "
                  f"Val Acc: {valacc:.3f}")
...
# Adding the text box
textstr = '\n'.join((
    f'{"Average Validation Loss:":<25}{0.041582:>10.6f}',
))
...
# Adding the text box
textstr = '\n'.join((
    f'{"Average Validation Accuracy:":<25}{0.982:>10.6f}',
))
props = dict(boxstyle='round', alpha=0.5, facecolor='white')
plt.gca().text(0.50, 0.65, textstr, transform=plt.gca().transAxes,
               fontsize=16, fontfamily='monospace', bbox=props)
plt.tight_layout()
plt.show()
