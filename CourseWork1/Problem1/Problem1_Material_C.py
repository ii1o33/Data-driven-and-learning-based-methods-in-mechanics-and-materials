...
############################################################################################
user_hidden = 344           #used for testing double hidden layer
user_width = 132
user_lr = 0.004741
user_BatchSize = 32
...
class Const_Net(nn.Module):
    def __init__(self, n_comp_in=1, n_comp_out=1, hidden=64, width=128, n_timestep=50):
...
#Normalise the data for strain (Fit the normalizer using only the training data and apply it to the test and validation data.)
...
#Normalise the data for stress (Fit the normalizer using only the training data and apply it to the test and validation data.)
...
############################################################################################
# For Material C: Only 1 strain and stress component used per timestep and per sample
net = Const_Net(n_comp_in=1, n_comp_out=1, hidden=user_hidden, width=user_width, n_timestep=50)
...
for epoch in range(epochs):
    ...
    #Early stopping when no further improvement (over 10 epochs) in validation loss
...
# Adding the text box
textstr = '\n'.join((
    f'{"Final Train Loss:":<25}{0.032408:>10.6f}',
    f'{"Final Validation Loss:":<25}{0.036402:>10.6f}',
    f'{"Final Test Loss:":<25}{0.038239:>10.6f}'
))
...
plt.figure(figsize=(12,8))
plt.plot(pred_stress[0, :, 0].numpy(), label='NN Prediction')  # Updated index for Material C (1 component)
