...
############################################################################################
user_hidden = 196       #used for testing double hidden layer
user_width = 97       
user_lr = 0.00340142
user_BatchSize = 32
...
class Const_Net(nn.Module):
    def __init__(self, n_comp_in=6, n_comp_out=6, hidden=64, width=128, n_timestep=50):
...
############################################################################################
# Data processing for Material B (all 6 strain and stress components)
############################################################################################
# Read data from .mat file
path = 'Material_B.mat'  # Updated for Material B
data_reader = MatRead(path)
strain = data_reader.get_strain()   # Use all 6 components for Material B
stress = data_reader.get_stress()   # Use all 6 components for Material B
...
val_strain = strain[ntrain:ntrain + nval]
val_stress = stress[ntrain:ntrain + nval]
test_strain = strain[ntrain + nval:]
test_stress = stress[ntrain + nval:]
...
#Normalise the data for stress (Fit the normalizer using only the training data and apply it to the test and validatino data.)
...
############################################################################################
# For Material B: All 6 strain and stress components used per timestep and per sample
net = Const_Net(n_comp_in=6, n_comp_out=6, hidden=user_hidden, width=user_width, n_timestep=50)
...
for epoch in range(epochs):
    ...
    for input_batch, target_batch in train_loader:
        ...
        optimizer.zero_grad()                               # Backprop and update
        loss.backward()
        optimizer.step()
        trainloss += loss.item()
...
#End timing and print CPU, wall and input/output read times    
...
# Adding the text box
textstr = '\n'.join((
    f'{"Final Train Loss:":<25}{0.005039:>10.6f}',
    f'{"Final Validation Loss:":<25}{0.008493:>10.6f}',
    f'{"Final Test Loss:":<25}{0.008867:>10.6f}'
))
...
plt.figure(figsize=(12,8))
plt.plot(pred_stress[0, :, 5].numpy(), label='NN Prediction')
plt.plot(true_stress[0, :, 5].numpy(), label='True Stress', linestyle='--')
