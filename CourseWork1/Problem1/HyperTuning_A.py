...
class Const_Net(nn.Module):
    def __init__(self, n_comp_in=3, n_comp_out=4, width=128, n_timestep=50):
...
############################################################################################
# Data processing accounting for plane strain condition (Material A)
############################################################################################
path = 'Material_A.mat'
...
train_strain, train_stress = strain_reduced[:ntrain], stress_reduced[:ntrain]
val_strain, val_stress = strain_reduced[ntrain:ntrain + nval], stress_reduced[ntrain:ntrain + nval]
test_strain, test_stress = strain_reduced[ntrain + nval:], stress_reduced[ntrain + nval:]
...
def objective(trial):
    ...
    net = Const_Net(n_comp_in=3, n_comp_out=4, width=width, n_timestep=50)
