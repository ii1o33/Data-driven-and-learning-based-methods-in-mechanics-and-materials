...
# Out of 400 samples, 300 samples are used for training and the rest for testing.
Ntotal     = 400                
train_size = 300
test_start = 300
...
def loss_func(y_approx, y_true):
    #For each sample in the batch, Compute the L2 error normalised by L2 norm of y_true
    #then averages over the batch dimension. This calulates the percentage error.

    eps = 1e-8  # to avoid division by zero
    
    # Numerator: sum of squared differences along time dimension
    num = torch.sum((y_approx - y_true)**2, dim=1)
    
    # Denominator: sum of squares of the truth (plus epsilon)
    den = torch.sum(y_true**2, dim=1) + eps
    
    # Ratio for each sample s
    ratios = torch.sqrt(num / den)
    
    # Average across the batch to get the final loss
...
# Normalize your data using the min-max normalizer (1e-8 to avoid dividing by zero)
data_input  = (data_input - data_input.min()) / (data_input.max() - data_input.min() + 1e-8)
data_output = (data_output - data_output.min()) / (data_output.max() - data_output.min() + 1e-8)
...
layer_hidden = [output_dim + n_hidden, 100, 100, 100, n_hidden]
