...
class CNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.unsqueeze(1)  # -> [batch_size, 1, Nx, Ny]

        # --- Encoder ---
        enc1 = self.encoder1(x)                 # [batch, features, Nx, Ny]
        enc2_in = self.pool1(enc1)              # downsample
        enc2_out = self.encoder2(enc2_in)       # [batch, features*2, Nx/2, Ny/2]
        enc3_in = self.pool2(enc2_out)
        enc3_out = self.encoder3(enc3_in)       # [batch, features*4, Nx/4, Ny/4]
        enc4_in = self.pool3(enc3_out)

        # --- Bottleneck ---
        bottleneck = self.bottleneck(enc4_in)   # [batch, features*8, Nx/8, Ny/8]

        # --- Decoder ---
        dec3 = self.up3(bottleneck)             # [batch, features*4, Nx/4, Ny/4]
        dec3 = torch.cat((dec3, enc3_out), dim=1)
        dec3 = self.decoder3(dec3)

        dec2 = self.up2(dec3)                   # [batch, features*2, Nx/2, Ny/2]
        dec2 = torch.cat((dec2, enc2_out), dim=1)
        dec2 = self.decoder2(dec2)

        dec1 = self.up1(dec2)                   # [batch, features, Nx, Ny]
        dec1 = torch.cat((dec1, enc1), dim=1)
        dec1 = self.decoder1(dec1)

        out = self.conv(dec1)                   # [batch, 1, Nx, Ny]
        out = out.squeeze(1)                    # [batch, Nx, Ny]
        return out
